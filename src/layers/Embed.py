...
class TokenEmbedding(nn.Module):
    def __init__(self, c_in: int, d_model: int, kernel_size: int = 3, spatial: bool = False) -> None:
        ...
        #apply special initialization (Kaiming Normal Initialization)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, L, d = x.shape[:3]
        if self.spatial: #only used for graph input
        ...
        else: #used for non-graph
            #apply 1d conv to input (shape: (batch_size, seq_len, c_in) --> (batch_size, seq_len, d_model))
            x = self.tokenConv(x.permute(0, 2, 1)).transpose(1, 2)[:, :L, :]
        return x
...
class DataEmbedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_mark: torch.Tensor, n_node: ... = None) -> torch.Tensor:
        #Apply conv1d (shape depends on whether or not x is graph data or not)
        # non-graph shape: (batch_size, seq_len, c_in) --> (batch_size, seq_len, d_model)
        val_embed = self.value_embedding(x)

        #Encodes temporal data (in one of three different ways, fixed, embeded, learned)
        temp_embed = self.temporal_embedding(x_mark) if self.temporal_embedding is not None else None
        
        #encodes the position of each datapoint in the sequence
