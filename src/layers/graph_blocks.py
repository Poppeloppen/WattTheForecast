...
class EdgeBlock(nn.Module):
    ...
    def forward(self, graph: GraphsTuple, **kwargs):
        ...
        #NOTE: this is just my own debugging stuff
        #for i, edge in enumerate(edges_to_collect):
        #    print(f"edges_to_collect[{i}]: {edge.shape}")
        #    print(f"Memory estimate: {sum([x.numel() for x in edges_to_collect]) * 4 / 1e9:.2f} GB")
        
        
        if self.num_edge_series == 1:
            collected_edges = torch.cat(edges_to_collect, dim=-1) #This throws memory error
